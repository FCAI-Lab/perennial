...
Definition NewLockedStackⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (return: (go.AllocValue LockedStack (let: "$stack" := ((FuncResolve go.make2 [go.SliceType go.string] #()) #(W64 0)) in
     CompositeLiteral LockedStack (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" (go.ZeroVal sync.Mutex #()) in
       let: "$$vs" := go.ElementListApp "$$vs" "$stack" in
       "$$vs"
     )))).
...
Definition LockedStack__Pushⁱᵐᵖˡ : val :=
  λ: "s" "value",
    ...
    do:  ((MethodResolve (go.PointerType sync.Mutex) Lock #() (StructFieldRef LockedStack "mu"%go (![go.PointerType LockedStack] "s"))) #());;;
    ...
    CompositeLiteral go.string go.string ["$sl0"])) in
    ...
    do:  ((MethodResolve (go.PointerType sync.Mutex) Unlock #() (StructFieldRef LockedStack "mu"%go (![go.PointerType LockedStack] "s"))) #());;;
    return: #()).
...
Definition LockedStack__Popⁱᵐᵖˡ : val :=
  λ: "s" <>,
    ...
    let: "$r0" := (![go.string] (slice.elem_ref go.string (![go.SliceType go.string] (StructFieldRef LockedStack "stack"%go (![go.PointerType LockedStack] "s"))) (![go.int] "last"))) in
    do:  ("v" <-[go.string] "$r0");;;
...
Definition NewEliminationStackⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (return: (go.AllocValue EliminationStack (let: "$base" := ((FuncResolve NewLockedStack [] #()) #()) in
     let: "$exchanger" := ((FuncResolve go.make1 [go.ChannelType go.sendrecv go.string] #()) #()) in
     CompositeLiteral EliminationStack (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" "$base" in
       let: "$$vs" := go.ElementListApp "$$vs" "$exchanger" in
       "$$vs"
     )))).
...
Definition EliminationStack__Pushⁱᵐᵖˡ : val :=
  λ: "s" "value",
    ...
    (MethodResolve (go.PointerType LockedStack) Push #() (![go.PointerType LockedStack] (StructFieldRef EliminationStack "base"%go (![go.PointerType EliminationStack] "s")))) "$a0");;;
    return: #()).
...
Definition EliminationStack__Popⁱᵐᵖˡ : val :=
  λ: "s" <>,
    ...
    let: ("$ret0", "$ret1") := (((MethodResolve (go.PointerType LockedStack) Pop #() (![go.PointerType LockedStack] (StructFieldRef EliminationStack "base"%go (![go.PointerType EliminationStack] "s")))) #())) in
    return: ("$ret0", "$ret1")).
...
Definition fib_consumerⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    chan.for_range go.int "$range" (λ: "$key",
      ...
      CompositeLiteral go.int go.int ["$sl0"])) in
...
Definition TestFibConsumerⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    let: "$sl8" := #(W64 21) in
    let: "$sl9" := #(W64 34) in
    CompositeLiteral go.int go.int ["$sl0"; "$sl1"; "$sl2"; "$sl3"; "$sl4"; "$sl5"; "$sl6"; "$sl7"; "$sl8"; "$sl9"])) in
    ...
    slice.for_range go.int "$range" (λ: "$key" "$value",
      do:  ("i" <-[go.int] "$key");;;
      (if: (![go.int] (slice.elem_ref go.int (![go.SliceType go.int] "result") (![go.int] "i"))) ≠⟨go.int⟩ (![go.int] (slice.elem_ref go.int (![go.SliceType go.int] "expected") (![go.int] "i")))
      then
        do:  (let: "$a0" := (InterfaceMake go.string #"incorrect output"%go) in
        (FuncResolve go.panic [] #()) "$a0")
      else do:  #())));;;
...
Definition mkRequestⁱᵐᵖˡ : val :=
  λ: "f",
    ...
    return: (let: "$f" := (![go.FunctionType (go.Signature [] #false [go.string])] "f") in
     let: "$result" := ((FuncResolve go.make2 [go.ChannelType go.sendrecv go.string] #()) #(W64 1)) in
     CompositeLiteral request (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" "$f" in
       let: "$$vs" := go.ElementListApp "$$vs" "$result" in
       "$$vs"
     ))).
...
Definition HigherOrderExampleⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    let: "$r0" := (let: "$a0" := (λ: <>,
      exception_do (return: (#("hello"%go +⟨go.string⟩ " world"%go)))
      ) in
    ...
    let: "$r0" := (let: "$a0" := (λ: <>,
      exception_do (return: (#(("w"%go +⟨go.string⟩ "o"%go) +⟨go.string⟩ "rld"%go)))
      ) in
    (FuncResolve mkRequest [] #()) "$a0") in
    do:  ("r3" <-[request] "$r0");;;
    ...
    let: "$r0" := ((let: "$sl0" := (Fst (chan.receive go.string (![go.ChannelType go.sendrecv go.string] (StructFieldRef request "result"%go "r1")))) in
    let: "$sl1" := (Fst (chan.receive go.string (![go.ChannelType go.sendrecv go.string] (StructFieldRef request "result"%go "r2")))) in
    let: "$sl2" := (Fst (chan.receive go.string (![go.ChannelType go.sendrecv go.string] (StructFieldRef request "result"%go "r3")))) in
    CompositeLiteral go.string go.string ["$sl0"; "$sl1"; "$sl2"])) in
...
Definition LeakyBufferPipelineⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    let: "$sl10" := #"l"%go in
    let: "$sl11" := #"d"%go in
    CompositeLiteral go.string go.string ["$sl0"; "$sl1"; "$sl2"; "$sl3"; "$sl4"; "$sl5"; "$sl6"; "$sl7"; "$sl8"; "$sl9"; "$sl10"; "$sl11"])) in
    let: "$a1" := (![go.ChannelType go.sendrecv (go.SliceType go.byte)] "freeList") in
    let: "$a2" := (![go.ChannelType go.sendrecv (go.SliceType go.byte)] "serverChan") in
...
Definition mkStreamⁱᵐᵖˡ : val :=
  λ: "f",
    ...
    return: (CompositeLiteral streamold (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" ((FuncResolve go.make1 [go.ChannelType go.sendrecv go.string] #()) #()) in
       let: "$$vs" := go.ElementListApp "$$vs" ((FuncResolve go.make1 [go.ChannelType go.sendrecv go.string] #()) #()) in
       let: "$$vs" := go.ElementListApp "$$vs" (![go.FunctionType (go.Signature [go.string] #false [go.string])] "f") in
       "$$vs"
     ))).
...
Definition Serveⁱᵐᵖˡ : val :=
  λ: "f",
    ...
    let: "$r0" := (let: "$req" := ((FuncResolve go.make1 [go.ChannelType go.sendrecv go.string] #()) #()) in
    let: "$res" := ((FuncResolve go.make1 [go.ChannelType go.sendrecv go.string] #()) #()) in
    CompositeLiteral stream (
      let: "$$vs" := go.StructElementListNil #() in 
      let: "$$vs" := go.ElementListApp "$$vs" "$req" in
      let: "$$vs" := go.ElementListApp "$$vs" "$res" in
      "$$vs"
    )) in
    do:  ("s" <-[stream] "$r0");;;
    let: "$go" := (λ: <>,
  ...
  λ: "c" "wg" "x" "y",
    ...
    do:  ("ok" <-[go.bool] "$r1")) := λ: <>,
      ...
      (FuncResolve go.len [go.SliceType go.int] #()) "$a0")); (λ: <>, do:  ("i" <-[go.int] ((![go.int] "i") +⟨go.int⟩ #(W8 1)))) := λ: <>,
        (if: (![go.int] (slice.elem_ref go.int (![go.SliceType go.int] "s") (![go.int] "i"))) =⟨go.int⟩ (![go.int] "x")
        then
          let: "$r0" := (![go.int] "y") in
