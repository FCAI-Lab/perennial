...
Definition arrayLiteralKeyedⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    let: "$r0" := ((let: "$ar0" := #"A"%go in
    let: "$ar1" := #"3"%go in
    let: "$ar2" := (go.ZeroVal go.string #()) in
    let: "$ar3" := (go.ZeroVal go.string #()) in
    let: "$ar4" := (go.ZeroVal go.string #()) in
    let: "$ar5" := (go.ZeroVal go.string #()) in
    let: "$ar6" := (go.ZeroVal go.string #()) in
    let: "$ar7" := (go.ZeroVal go.string #()) in
    let: "$ar8" := (go.ZeroVal go.string #()) in
    let: "$ar9" := (go.ZeroVal go.string #()) in
    let: "$ar10" := #"B"%go in
    let: "$ar11" := #"1"%go in
    let: "$ar12" := #"2"%go in
...
Definition chanSelectⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    chan.select_nonblocking [chan.select_receive go.int (![go.ChannelType go.sendrecv go.int] "c3") (λ: "$recvVal",
       ...
       "%go) in
       CompositeLiteral (go.InterfaceType []) (go.InterfaceType []) ["$sl0"; "$sl1"; "$sl2"])) in
       ...
       then
         ...
         "%go) in
         CompositeLiteral (go.InterfaceType []) (go.InterfaceType []) ["$sl0"])) in
         (FuncResolve fmt.Print [] #()) "$a0"))
...
Definition testNumWrapperⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    let: "$r0" := #(W64 0) in
    do:  ("n" <-[numWrapper] "$r0");;;
    do:  ((MethodResolve (go.PointerType numWrapper) inc #() "n") #());;;
    return: #()).
...
Definition iterMapKeysⁱᵐᵖˡ : val :=
  λ: "m",
    ...
    map.for_range "$range" (λ: "$key" "value",
      ...
      CompositeLiteral go.uint64 go.uint64 ["$sl0"])) in
...
Definition returnEmbedValⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (return: (CompositeLiteral embedB (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" (go.ZeroVal embedA #()) in
       "$$vs"
     ))).
...
Definition returnEmbedValWithPointerⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (return: (CompositeLiteral embedD (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" (go.ZeroVal embedC #()) in
       "$$vs"
     ))).
...
Definition useEmbeddedFieldⁱᵐᵖˡ : val :=
  λ: "d",
    ...
    let: "$r0" := (go.AllocValue embedD (CompositeLiteral embedD (
      let: "$$vs" := go.StructElementListNil #() in 
      let: "$$vs" := go.ElementListApp "$$vs" (go.ZeroVal embedC #()) in
      "$$vs"
    ))) in
...
Definition useEmbeddedMethodⁱᵐᵖˡ : val :=
  λ: "d",
    exception_do (let: "d" := (go.AllocValue embedD "d") in
    return: (((MethodResolve embedD Foo #() (![embedD] "d")) #()) =⟨go.uint64⟩ ((MethodResolve embedA Foo #() (![embedA] (StructFieldRef embedB "embedA"%go (![go.PointerType embedB] (StructFieldRef embedC "embedB"%go (StructFieldRef embedD "embedC"%go "d")))))) #()))).
...
Definition useEmbeddedMethod2ⁱᵐᵖˡ : val :=
  λ: "d",
    ...
    return: (((MethodResolve embedD Bar #() (![embedD] "d")) #()) =⟨go.uint64⟩ ((MethodResolve (go.PointerType embedB) Bar #() (![go.PointerType embedB] (StructFieldRef embedC "embedB"%go (StructFieldRef embedD "embedC"%go "d")))) #()))).
...
Definition fooConsumerⁱᵐᵖˡ : val :=
  λ: "f",
    ...
    do:  ((MethodResolve Fooer Foo #() (![Fooer] "f")) #());;;
    return: #()).
...
Definition testAssignConcreteToInterfaceⁱᵐᵖˡ : val :=
  λ: "x",
    ...
    let: "$r0" := (go.AllocValue concreteFooer (CompositeLiteral concreteFooer (
      let: "$$vs" := go.StructElementListNil #() in 
...
Definition testPassConcreteToInterfaceArgSpecialⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    CompositeLiteral Fooer Fooer ["$sl0"; "$sl1"])) in
    ...
    let: "$r0" := (CompositeLiteral FooerUser (
      let: "$$vs" := go.StructElementListNil #() in 
      let: "$$vs" := go.ElementListApp "$$vs" (InterfaceMake (go.PointerType concreteFooer) (![go.PointerType concreteFooer] "c1")) in
      "$$vs"
    )) in
    do:  ("f" <-[FooerUser] "$r0");;;
...
Definition takesVarArgsInterfaceⁱᵐᵖˡ : val :=
  λ: "fs",
    ...
    do:  ((MethodResolve Fooer Foo #() (![Fooer] (slice.elem_ref Fooer (![go.SliceType Fooer] "fs") #(W64 0)))) #());;;
    return: #()).
...
Definition testⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (do:  (let: "$a0" := ((let: "$sl0" := (InterfaceMake (go.PointerType concreteFooer) (go.AllocValue concreteFooer (CompositeLiteral concreteFooer (
      let: "$$vs" := go.StructElementListNil #() in 
      let: "$$vs" := go.ElementListApp "$$vs" (go.ZeroVal go.uint64 #()) in
      "$$vs"
    )))) in
    let: "$sl1" := (InterfaceMake (go.PointerType concreteFooer) (go.AllocValue concreteFooer (CompositeLiteral concreteFooer (
      let: "$$vs" := go.StructElementListNil #() in 
      let: "$$vs" := go.ElementListApp "$$vs" (go.ZeroVal go.uint64 #()) in
      "$$vs"
    )))) in
...
Definition returnConcreteⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (return: (go.AllocValue concreteFooer (CompositeLiteral concreteFooer (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" (go.ZeroVal go.uint64 #()) in
       "$$vs"
     )), #(W64 10))).
...
Definition giveMultipleⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (return: (#(W64 0), InterfaceMake (go.PointerType concreteFooer) (go.AllocValue concreteFooer (CompositeLiteral concreteFooer (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" (go.ZeroVal go.uint64 #()) in
       "$$vs"
     ))), go.AllocValue concreteFooer (CompositeLiteral concreteFooer (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" (go.ZeroVal go.uint64 #()) in
       "$$vs"
     )))).
...
Definition testPtrMsetⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    let: "$r0" := (go.AllocValue concrete1 (CompositeLiteral concrete1 (
      let: "$$vs" := go.StructElementListNil #() in 
      "$$vs"
    ))) in
    ...
    do:  ("f" <-[Fooer] "$r0");;;
    do:  ((MethodResolve PointerInterface B #() (![PointerInterface] "p")) #());;;
    do:  ((MethodResolve Fooer Foo #() (![Fooer] "f")) #());;;
    return: #()).
...
Definition signedMidpointⁱᵐᵖˡ : val :=
  λ: "x" "y",
    ...
    return: (((![go.int] "x") +⟨go.int⟩ (![go.int] "y")) `quot`⟨go.int⟩ #(W64 2))).
...
Definition normalLiteralsⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (return: (let: "$int" := #(W64 0) in
     let: "$s" := #"foo"%go in
     let: "$b" := #true in
     CompositeLiteral allTheLiterals (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" "$int" in
...
Definition unKeyedLiteralⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (return: (CompositeLiteral allTheLiterals (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" #(W64 0) in
       let: "$$vs" := go.ElementListApp "$$vs" #"a"%go in
       let: "$$vs" := go.ElementListApp "$$vs" #false in
       "$$vs"
     ))).
...
Definition intSliceLoopⁱᵐᵖˡ : val :=
  λ: "xs",
    ...
    (FuncResolve go.len [go.SliceType go.uint64] #()) "$a0")); (λ: <>, do:  ("i" <-[go.int] ((![go.int] "i") +⟨go.int⟩ #(W8 1)))) := λ: <>,
      do:  ("sum" <-[go.uint64] ((![go.uint64] "sum") +⟨go.uint64⟩ (![go.uint64] (slice.elem_ref go.uint64 (![go.SliceType go.uint64] "xs") (![go.int] "i")))))));;;
    return: (![go.uint64] "sum")).
...
Definition useNilFieldⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (return: (go.AllocValue containsPointer (let: "$s" := #null in
     CompositeLiteral containsPointer (
       let: "$$vs" := go.StructElementListNil #() in 
       let: "$$vs" := go.ElementListApp "$$vs" "$s" in
       "$$vs"
     )))).
...
Definition wrapExternalStruct__joinⁱᵐᵖˡ : val :=
  λ: "w" <>,
    ...
    do:  ((MethodResolve (go.PointerType std.JoinHandle) Join #() (![go.PointerType std.JoinHandle] (StructFieldRef wrapExternalStruct "j"%go "w"))) #());;;
    return: #()).
...
Definition RecursiveEmbedded__recurEmbeddedMethodⁱᵐᵖˡ : val :=
  λ: "r" <>,
    ...
    do:  ((MethodResolve Other recurEmbeddedMethod #() (![Other] (StructFieldRef RecursiveEmbedded "Other"%go (![go.PointerType RecursiveEmbedded] "r")))) #());;;
    return: #()).
...
Definition sliceOpsⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    return: ((((((![go.uint64] "v1") +⟨go.uint64⟩ (![go.uint64] (slice.elem_ref go.uint64 (![go.SliceType go.uint64] "v2") #(W64 0)))) +⟨go.uint64⟩ (![go.uint64] (slice.elem_ref go.uint64 (![go.SliceType go.uint64] "v3") #(W64 1)))) +⟨go.uint64⟩ (![go.uint64] (![go.PointerType go.uint64] "v4"))) +⟨go.uint64⟩ (s_to_w64 (let: "$a0" := (![go.SliceType go.uint64] "x") in
...
Definition makeSingletonSliceⁱᵐᵖˡ : val :=
  λ: "x",
    ...
    return: ((let: "$sl0" := (![go.uint64] "x") in
     CompositeLiteral go.uint64 go.uint64 ["$sl0"]))).
...
Definition sliceOfThings__getThingRefⁱᵐᵖˡ : val :=
  λ: "ts" "i",
    ...
    return: (slice.elem_ref thing (![go.SliceType thing] (StructFieldRef sliceOfThings "things"%go "ts")) (![go.uint64] "i"))).
...
Definition xⁱᵐᵖˡ : val :=
  λ: <>,
    exception_do (do:  (let: "$a0" := #("a"%go +⟨go.string⟩ "b"%go) in
...
Definition testVariadicCallⁱᵐᵖˡ : val :=
  λ: <>,
    ...
    let: "$sl2" := #(W8 2) in
    let: "$sl3" := #(W8 3) in
    CompositeLiteral go.byte go.byte ["$sl0"; "$sl1"; "$sl2"; "$sl3"])) in
